...
//any tmeporary email or password 
...
let page, browser;//page apna tab hoga 
browserStartPromise
    ...
    function waitAndClick(selector, cPage) {//selcetor and tab mila 
    return new Promise(function (resolve, reject) {
        //inbuilt function hai waitForSelector ka ,wait some time for page to load 
        let waitForModalPromise = cPage.waitForSelector(selector, { visible: true });
        waitForModalPromise
            .then(function () {//selector is now avaialaible 
                let clickModal =
                    cPage.click(selector, { delay: 100 });//click it 
                return clickModal;
            }).then(function () {
                resolve();//no problem promise fulfilled 
            }).catch(function (err) {
                reject(err)
            })
    }
